...
pub const PLUGIN_DOWNLOAD_URL: &str =
    "https://github.com/Eszuri/symvonia/releases/latest/download/symvonia-tag-editor.exe";

pub const PLUGIN_EXPECTED_SHA256: &str = match option_env!("SYMVONIA_TAG_EDITOR_PLUGIN_SHA256") {
    Some(value) => value,
    None => "",
};

pub const PLUGIN_EXE_NAME: &str = "symvonia-tag-editor.exe";
...
pub fn plugin_dir(app: &AppHandle) -> Result<PathBuf, String> {
    ...
    Ok(app_data.join("plugins").join("tag-editor"))
}
...
pub fn download_and_install(app: &AppHandle, custom_url: Option<String>) -> Result<PluginStatus, String> {
    ...
    loop {
        if DOWNLOAD_CANCELLED.load(Ordering::SeqCst) {
            ...
            let _ = app.emit("tag-editor-download-cancelled", ());
    ...
    serde_json::from_str::<VerifyResponse>(line)
        .map(|response| {
            response.event == "verify_response"
                ...
                && response.engine == "Symvonia Tag Editor Engine"
...
pub fn verify_plugin_executable(src: &Path) -> Result<(), String> {
    ...
    if !(500 * 1024..=500 * 1024 * 1024).contains(&len) {
        return Err(format!(
            "Ukuran berkas ({:.2} MB) tidak valid untuk plugin Symvonia Tag Editor (harus 0.5 MB - 500 MB).",
    ...
    let nonce = std::time::SystemTime::now()
        ...
        .unwrap_or(123456789);
    let token = format!("symvonia_tag_token_{nonce:x}");
    ...
    Err("Verifikasi gagal: file bukan plugin Symvonia Tag Editor Engine yang valid.".into())
}
...
pub fn write_tags_via_plugin(
    ...
    let exe = plugin_exe_path(app)?;
    if !exe.exists() {
        return Err("Plugin Tag Editor belum terinstall. Silakan pasang plugin Tag Editor di Pengaturan.".into());
    }
    ...
    let mut child = command.spawn().map_err(|e| format!("Gagal menjalankan Tag Editor plugin: {}", e))?;
    ...
    if let Some(stdout) = child.stdout.take() {
        ...
        if let Ok(res) = serde_json::from_str::<WriteResultEvent>(&line) {
            if res.success {
                ...
                return Err(res.error.unwrap_or_else(|| "Gagal menyimpan tag metadata".into()));
            }
        }
    } else {
        let _ = child.kill();
    ...
    Err("Tidak ada respon dari plugin Tag Editor".into())
}
...
mod tests {
    ...
    fn test_plugin_exe_name() {
        assert_eq!(PLUGIN_EXE_NAME, "symvonia-tag-editor.exe");
    }

    #[test]
    fn verify_response_requires_tag_editor_identity() {
        let token = "test_token_456";
        let valid_json = format!(
            r#"{{"event":"verify_response","token":"{}","engine":"Symvonia Tag Editor Engine","version":"1.0.0"}}"#,
            token
        );
        assert!(verify_response_line(&valid_json, token));

        let wrong_engine_json = format!(
            r#"{{"event":"verify_response","token":"{}","engine":"Symvonia AI Lyrics Engine","version":"1.0.0"}}"#,
            token
        );
    ...
    #[test]
    fn test_verify_real_published_tag_editor_exe() {
        let published_exe = Path::new("../plugin/src-tag-editor/publish/symvonia-tag-editor.exe");
