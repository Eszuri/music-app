...
export default function DebugSection({lang, logs}: {lang: Lang; logs: LogEntry[]}) {
    ...
    return (
        <div className="h-full flex flex-col">
            <div className="text-xs text-zinc-500 mb-2">
                {t(lang, 'debug.totalLog')} {logs.length}
            </div>
            <div className="flex-1 overflow-y-auto bg-black/40 rounded-xl border border-zinc-800 p-3 font-mono text-[11px] leading-relaxed space-y-1">
                {logs.length === 0 ? (
                    <div className="text-zinc-600 italic">{t(lang, 'debug.empty')}</div>
                ) : (
                    logs.map((log) => (
