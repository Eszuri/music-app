...
export default function LyricsSection({
    ...
    const [defaultModel, setDefaultModel] = useState<string>('base');
    ...
    useEffect(() => {
        if (typeof window !== 'undefined') {
            const saved = localStorage.getItem('symvonia_ai_lyrics_model');
    ...
    const handleSetDefaultModel = (code: string) => {
        setDefaultModel(code);
        if (typeof window !== 'undefined') {
            localStorage.setItem('symvonia_ai_lyrics_model', code);
        }
    };
    ...
    const openManualGuideForModel = (code: string) => {
        setSelectedManualModelCode(code);
        ...
        <div className="space-y-6 pb-6">
            ...
            <div className="flex items-center justify-between px-1 mb-5">
                <div className="flex items-center gap-3.5">
                    <div className="p-2.5 bg-purple-950/40 rounded-2xl border border-purple-800/40 shadow-md flex items-center justify-center shrink-0">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-zinc-400/90 mt-0.5 leading-relaxed">
                ...
                <div className="flex items-center gap-2">
                    <button
                        ...
                        className="px-3 py-1.5 text-xs font-semibold rounded-xl bg-zinc-800/80 hover:bg-zinc-700 text-zinc-200 border border-zinc-700/60 transition-all flex items-center gap-1.5 cursor-pointer shadow-xs active:scale-95"
                    ...
                    <button
                        ...
                        className={`px-3 py-1.5 text-xs font-semibold rounded-xl transition-all flex items-center gap-1.5 cursor-pointer shadow-xs active:scale-95 ${
                            showManualGuide
                    ...
                    >
                        {showManualGuide ? (
                            <>
                                ...
                                <span>{t(lang, 'lyrics.manager.showListBtn')}</span>
                            </>
                        ) : (
                            <>
                                <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                                ...
                                <span>{t(lang, 'lyrics.manager.manualGuideBtn')}</span>
                            </>
                        )}
                    </button>
                </div>
            ...
            {!isInstalled && (
                <div className="p-4 rounded-2xl bg-amber-500/10 border border-amber-500/25 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            ...
            {/* System Specs & Real-time Stats Header */}
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div className="p-3.5 rounded-2xl bg-zinc-900/80 border border-zinc-800/80 flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <div className="text-sm font-bold text-zinc-100 mt-0.5">
                            {downloadedModels.length} / {AI_MODELS_LIST.length} {t(lang, 'lyrics.manager.models')}
                        </div>
                    </div>
                </div>
                ...
                <div className="p-3.5 rounded-2xl bg-zinc-900/80 border border-zinc-800/80 flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <div className="text-sm font-bold text-zinc-100 mt-0.5">
                            {downloadedModels.length > 0 ? totalSizeText : '0 GB'}
                        </div>
                    </div>
                </div>
                ...
                <div className="p-3.5 rounded-2xl bg-zinc-900/80 border border-zinc-800/80 flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <div className="text-xs font-semibold text-zinc-200 mt-0.5">
                            {systemSpecs ? `${systemSpecs.ramGb}GB RAM • ${systemSpecs.cpuCores} Core CPU` : 'Detecting...'}
                        </div>
                    </div>
                </div>
            </div>

            {/* Mutually Exclusive Rendering: Manual Guide vs Model Downloader Manager Cards */}
            {showManualGuide ? (
                /* Manual Installation Guide Section */
                <div className="p-5 rounded-2xl bg-zinc-900/95 border border-purple-500/30 space-y-4 animate-in fade-in duration-200">
                    <div className="flex items-center justify-between border-b border-zinc-800 pb-3">
                        ...
                        <button
                            ...
                            className="px-2.5 py-1 text-xs font-semibold rounded-lg bg-zinc-800 hover:bg-zinc-700 text-zinc-300 transition-all cursor-pointer flex items-center gap-1 border border-zinc-700/60 shadow-xs active:scale-95"
                        >
                    ...
                    <div className="space-y-1.5 text-xs text-zinc-300 leading-relaxed">
                        ...
                        <div className="p-2.5 rounded-xl bg-zinc-950/80 border border-zinc-800/80 font-mono text-[11px] text-zinc-400 space-y-1">
                            <div className="text-purple-400 font-semibold">
                                {t(lang, 'lyrics.manual.targetDir')}:
                            </div>
                            <div className="select-all text-zinc-200 break-all bg-zinc-900 p-2 rounded-lg border border-zinc-800/60">
                                %APPDATA%\com.symvonia.player\plugins\ai-lyrics\models\
                            </div>
                        </div>
                    </div>

                    {/* Per-Model Selector Tabs */}
                    <div className="space-y-3 pt-2 border-t border-zinc-800/60">
                        <div className="flex items-center justify-between">
                            ...
                            <span className="text-[11px] font-mono text-purple-400 font-semibold">
                                ggml-{selectedManualModel.code}.bin ({selectedManualModel.sizeText})
                            </span>
                        </div>
                        ...
                        <div className="p-4 rounded-xl bg-zinc-950/90 border border-zinc-800 space-y-3">
                            ...
                            <div className="space-y-1">
                                <div className="flex items-center justify-between text-[11px]">
                                    ...
                                    >
                                        {copiedLink ? t(lang, 'lyrics.manual.copied') : t(lang, 'lyrics.manual.copyLink')}
                                    </button>
                                </div>
                                <div className="p-2 rounded-lg bg-zinc-900 border border-zinc-800 font-mono text-[10px] text-zinc-300 break-all select-all">
                            ...
                            <div className="space-y-1 pt-1">
                                <div className="flex items-center justify-between text-[11px]">
                                    <span className="text-zinc-400 font-medium">PowerShell Command (Model {selectedManualModel.label}):</span>
                                    <button
                                        onClick={copyScript}
                                        className="px-2.5 py-1 text-[11px] font-semibold rounded-lg bg-emerald-500/20 text-emerald-300 hover:bg-emerald-500/30 transition-all cursor-pointer"
                                    >
                                        {copiedCmd ? t(lang, 'lyrics.manual.copied') : t(lang, 'lyrics.manual.copyPsCmd')}
                                    </button>
                                </div>
                                <pre className="p-3 rounded-xl bg-zinc-900 border border-zinc-800 text-[10px] text-emerald-400 font-mono overflow-x-auto whitespace-pre-wrap select-all">
                                    {currentPsScript}
                                </pre>
                            </div>
                        </div>
                ...
                /* Model Downloader Manager Cards */
                <SettingGroup title={t(lang, 'lyrics.manager.groupTitle')}>
                    <div className="divide-y divide-zinc-800/60">
                        ...
                        const isDefault = defaultModel === model.code;
                        ...
                        return (
                            <div key={model.code} className="py-4 first:pt-2 last:pb-2 space-y-3">
                                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                                    ...
                                    <div className="space-y-1">
                                        <div className="flex items-center gap-2 flex-wrap">
                                            ...
                                            {isDownloaded ? (
                                                ...
                                                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold bg-zinc-800/80 text-zinc-400 border border-zinc-700/50">
                                            ...
                                            {/* System Spec Recommendation Badge */}
                                            {!isRec && (
                                    ...
                                    <div className="flex items-center gap-2 shrink-0 self-end sm:self-center flex-wrap">
                                        {/* Per-Model Manual Info Button */}
                                        <button
                                            onClick={() => openManualGuideForModel(model.code)}
                                            className="px-2.5 py-1.5 text-xs font-semibold rounded-xl bg-purple-950/20 hover:bg-purple-900/30 text-purple-300 border border-purple-800/30 transition-all cursor-pointer flex items-center gap-1 active:scale-95"
                                            title={t(lang, 'lyrics.manual.modelSpecificGuide', { model: model.label })}
                                        >
                                            ...
                                            <span>{t(lang, 'lyrics.manager.manualModelBtn')}</span>
                                        </button>

                                        {/* Set Default Button */}
                                        {isDownloaded && !isDefault && (
                                            <button
                                                onClick={() => handleSetDefaultModel(model.code)}
                                                className="px-3 py-1.5 text-xs font-semibold rounded-xl bg-purple-950/30 hover:bg-purple-900/40 text-purple-300 border border-purple-800/40 transition-all cursor-pointer active:scale-95"
                                            >
                                                {t(lang, 'lyrics.manager.setDefaultBtn')}
                                            </button>
                                        )}
                                        ...
                                        {!isDownloaded ? (
                                            <div className="flex items-center gap-2">
                                                <button
                                                    ...
                                                    className="px-3 py-1.5 text-xs font-semibold rounded-xl bg-zinc-800/90 hover:bg-zinc-700 text-zinc-300 border border-zinc-700/60 transition-all cursor-pointer flex items-center gap-1 active:scale-95"
                                                ...
                                                <button
                                                    ...
                                                    className={`px-3.5 py-1.5 text-xs font-bold rounded-xl ${accent.bg500} text-white hover:opacity-95 transition-all flex items-center gap-1.5 cursor-pointer shadow-xs active:scale-95 disabled:opacity-50`}
                                                >
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                                                        <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/>
                                                        <polyline points="7 10 12 15 17 10"/>
                                            ...
                                            <button
                                                ...
                                                className="px-3 py-1.5 text-xs font-semibold rounded-xl bg-rose-500/10 text-rose-400 hover:bg-rose-500/20 border border-rose-500/25 transition-all cursor-pointer flex items-center gap-1 active:scale-95"
                                            >
                                                <svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                                ...
                                {isDownloadingThisModel && modelDownloadProgress && (
                                    <div className="p-3 rounded-xl bg-zinc-950/80 border border-purple-500/30 space-y-2 animate-in fade-in duration-200">
