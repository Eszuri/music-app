...
/**
 * Base Skeleton component — shimmer via CSS @keyframes (zero JS animation frames).
 */
...
export function VolumeControlSkeleton({ accentColor = "violet" }: { accentColor?: string }) {
  return (
    <div className="flex items-center gap-2 w-full justify-center">
      ...
      <Skeleton accentColor={accentColor} className="flex-1 min-w-[56px] max-w-40 h-1 rounded-full" />
      <Skeleton accentColor={accentColor} variant="button" className="w-5 h-5" />
...
/**
 * Skeleton for MetadataPanel — plain div, no enter/exit animation needed.
 */
export function MetadataPanelSkeleton({ accentColor = "violet" }: { accentColor?: string }) {
  return (
    <div className="space-y-4">
      <Skeleton accentColor={accentColor} variant="cover" className="w-full max-w-[160px] mx-auto ring-1 ring-white/5" />
  ...
  return (
    <header className="flex items-center px-3 sm:px-5 py-2.5 sm:py-3 border-b border-zinc-800/50 bg-zinc-950/80">
      <div className="flex items-center gap-1.5 shrink-0">
        <Skeleton accentColor={accentColor} variant="button" className="w-[72px] h-7 rounded-lg" />
        <Skeleton accentColor={accentColor} variant="button" className="w-[72px] h-7 rounded-lg" />
      </div>
      <div className="flex-1 flex justify-center">
        <Skeleton accentColor={accentColor} variant="text" className="h-5 w-32" />
      </div>
      <Skeleton accentColor={accentColor} variant="button" className="w-[68px] h-5 rounded-full shrink-0" />
    </header>
  );
}
