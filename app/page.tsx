...
function HomeContent() {
    ...
    const [globalContextMenu, setGlobalContextMenu] = useState<{x: number; y: number; items: ContextMenuItem[]} | null>(null);
    ...
    // Prevent native browser context menu globally
    useEffect(() => {
        const preventNative = (e: MouseEvent) => e.preventDefault();
        document.addEventListener('contextmenu', preventNative, true);
        return () => document.removeEventListener('contextmenu', preventNative, true);
    }, []);

    // Block Ctrl+R (browser reload shortcut)
    useEffect(() => {
        const blockReload = (e: KeyboardEvent) => {
            if ((e.ctrlKey || e.metaKey) && e.key == 'r'|| (e.ctrlKey || e.metaKey) && e.key == 'R') {
                e.preventDefault();
            }
        };
    ...
    /**
     * Global context menu — placed on elements that don't have their own specific handler.
     * Does NOT fire when Settings or Streaming modal is open.
     */
    const showGlobalContextMenu = useCallback((e: React.MouseEvent) => {
        // Suppress when any modal is open
        ...
        const items: ContextMenuItem[] = [
            ...
            },
            {
                ...
                icon: <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polygon points="5 4 15 12 5 20 5 4"/><line x1="19" y1="5" x2="19" y2="19"/></svg>,
                onClick: player.playNext,
                disabled: !hasSong,
            },
            {
                label: t(lang, 'contextMenu.prev'),
                icon: <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polygon points="19 20 9 12 19 4 19 20"/><line x1="5" y1="19" x2="5" y2="5"/></svg>,
            ...
            {separator: true} as ContextMenuItem,
            {
                ...
                icon: <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/><path d="M19.07 4.93a10 10 0 0 1 0 14.14"/><path d="M15.54 8.46a5 5 0 0 1 0 7.07"/></svg>,
                onClick: () => {
                    const step = settings.volumeStep / 100;
                    const newVol = Math.min(1, player.activeVolume + step);
                    player.handleVolumeChange({target: {value: String(newVol)}} as ChangeEvent<HTMLInputElement>);
                },
            },
            {
                label: t(lang, 'contextMenu.decreaseVolume'),
                icon: <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/><line x1="23" y1="9" x2="17" y2="15"/><line x1="17" y1="9" x2="23" y2="15"/></svg>,
                onClick: () => {
                    const step = settings.volumeStep / 100;
            ...
            {separator: true} as ContextMenuItem,
            {
                ...
                badge: settings.shuffle ? t(lang, 'playback.shuffleOn').split(':')[1]?.trim() ?? 'ON' : undefined,
            },
            {
                label: settings.repeat === 'off'
                    ? t(lang, 'playback.repeatOff')
                    : settings.repeat === 'all'
                        ? t(lang, 'playback.repeatAll')
                        : t(lang, 'playback.repeatOne'),
                icon: <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="17 1 21 5 17 9"/><path d="M3 11V9a4 4 0 0 1 4-4h14"/><polyline points="7 23 3 19 7 15"/><path d="M21 13v2a4 4 0 0 1-4 4H3"/></svg>,
                onClick: () => {
            ...
            },
            {
                ...
                icon: <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M4 11a9 9 0 0 1 9 9"/><path d="M4 4a16 16 0 0 1 16 16"/><circle cx="5" cy="19" r="1"/></svg>,
                onClick: () => setStreamingOpen(true),
            },
        ];
        setGlobalContextMenu({x: e.clientX, y: e.clientY, items: [...items, ...appendDevTools(items, lang)]});
    }, [
        lang,
        player.selectedSong,
        player.isPlaying,
