...
export function getPresetGains(bandMode: EQBandMode, presetKey: EQPresetKey): number[] {
    ...
    // Interpolate / map gains based on relative position 0..1 across frequency range
    return EQ_BAND_FREQUENCIES[bandMode].map((freq) => {
        if (presetKey === 'bassBoost') {
            if (freq <= 100) return 6;
            if (freq <= 250) return 4;
            if (freq <= 500) return 2;
            return 0;
        }
        if (presetKey === 'trebleBoost') {
            if (freq >= 8000) return 7;
            if (freq >= 4000) return 5;
            if (freq >= 2000) return 3;
            if (freq >= 1000) return 1;
            return 0;
        }
        if (presetKey === 'rock') {
            if (freq <= 64) return 5;
            if (freq <= 125) return 3;
            if (freq <= 500) return 0;
            if (freq <= 2000) return 1;
            if (freq <= 8000) return 4;
            return 6;
        }
        if (presetKey === 'pop') {
            if (freq <= 100) return 2;
            if (freq <= 500) return 4;
            if (freq <= 2000) return 2;
            if (freq <= 8000) return 3;
            return 2;
        }
        if (presetKey === 'jazz') {
        ...
        if (presetKey === 'electronic') {
            if (freq <= 64) return 6;
            if (freq <= 250) return 4;
            if (freq <= 1000) return -2;
            if (freq <= 4000) return 2;
            if (freq <= 10000) return 5;
            return 6;
        }
        if (presetKey === 'vocal') {
            if (freq <= 125) return -3;
...
function loadSavedEqualizer(): EqualizerState {
    ...
    try {
        ...
        return {enabled, bandMode, preampDb, preset, gains};
    } catch {
